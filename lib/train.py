...
try:
    ...
    print("[WARNING] Tensorflow Hub not available.")
...
def run_final_eval(train_session, module_spec, class_count, image_lists,
                   jpeg_data_tensor, decoded_image_tensor,
                   resized_image_tensor, bottleneck_tensor, learning_rate, test_batch_size, bottleneck_dir, image_dir, tfhub_module, final_tensor_name, print_misclassified_test_images):
    ...
    (eval_session, _, bottleneck_input, ground_truth_input, evaluation_step,
     prediction) = build_eval_session(module_spec, class_count, learning_rate, final_tensor_name)
...
def build_eval_session(module_spec, class_count, learning_rate, final_tensor_name):
...
def save_graph_to_file(graph, graph_file_name, module_spec, class_count, learning_rate, final_tensor_name):
    """Saves an graph to file, creating a valid quantized one if necessary."""
    sess, _, _, _, _, _ = build_eval_session(
        module_spec, class_count, learning_rate, final_tensor_name)
    graph = sess.graph
...
def export_model(module_spec, class_count, saved_model_dir, learning_rate, final_tensor_name):
...
def train_model(image_dir, tfhub_module, testing_percentage=10, validation_percentage=10, flip_left_right=False, random_crop=0, random_scale=0, random_brightness=0, final_tensor_name="final_result", bottleneck_dir="model/bottleneck", summaries_dir="model/retrain_logs", how_many_training_steps=4000, train_batch_size=100, eval_step_interval=10, validation_batch_size=100, intermediate_store_frequency=0, intermediate_output_graphs_dir="model/intermediate_graph", output_graph="model/output_graph.pb", output_labels="model/output_labels.txt", saved_model_dir="model/$(date +%s)", learning_rate=0.01, test_batch_size=-1, print_misclassified_test_images=False):
    ...
    prepare_file_system(
        summaries_dir, intermediate_store_frequency, intermediate_output_graphs_dir)
    ...
    with graph.as_default():
        (train_step, cross_entropy, bottleneck_input,
         ground_truth_input, final_tensor) = add_final_retrain_ops(
             class_count, final_tensor_name, bottleneck_tensor,
             wants_quantization, learning_rate, is_training=True)
    ...
    with tf.Session(graph=graph) as sess:
        ...
        for i in range(how_many_training_steps):
            ...
            if (intermediate_frequency > 0 and (i % intermediate_frequency == 0)
                ...
                save_graph_to_file(graph, intermediate_file_name, module_spec,
                                   class_count, learning_rate, final_tensor_name)
        ...
        run_final_eval(sess, module_spec, class_count, image_lists,
                       ...
                       bottleneck_tensor, learning_rate, test_batch_size, bottleneck_dir, image_dir, tfhub_module, final_tensor_name, print_misclassified_test_images)
        ...
        save_graph_to_file(graph, output_graph, module_spec,
                           class_count, learning_rate, final_tensor_name)
        ...
        if saved_model_dir:
            export_model(module_spec, class_count, saved_model_dir,
                         learning_rate, final_tensor_name)


def train(config, saved_model_dir):
    """Use transfer learning and fine-tuning to train a network on a new dataset"""
    image_dir = config.get("tensorflow", "labeled_images")
    tfhub_module = config.get("tensorflow", "hub_model")
    train_model(image_dir=image_dir, saved_model_dir=saved_model_dir,
